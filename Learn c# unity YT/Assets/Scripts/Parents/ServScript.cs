...
public class ServScript : MonoBehaviour {
    ...
    private float pointerStart_Key = 310;
    private float pointerEnd_Key = 50;
    ...
    void Start() // On Game Start
    {
        ...
        initialOnce = false; //-enable only once
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        ...
        if (other.gameObject.tag == fiducialTag && other.bounds.Contains(collBounds))
        {
            ...
            if (now > pointerStart_Key || now < pointerEnd_Key
            ) //if tangible angle is within range & clip tangible rotation to pointer limits
            {
